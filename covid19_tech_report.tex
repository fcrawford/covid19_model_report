...
\usepackage{setspace}
\usepackage[colorlinks,linkcolor=black,citecolor=black,urlcolor=blue,backref=page]{hyperref}
...
Figure \ref{fig:fit} shows calibrated projections of hospitalizations and deaths, with 95\% uncertainty intervals, under the low asymptomatic fraction scenario and observed data as dots.  Calibration results under medium and high asymptomatic fraction scenarios demonstrate a similar fit to historical data.  Figures \ref{fig:slow_low} to \ref{fig:fast_high} show projections of under the three different scenarios with `slow' reopening. The projections include daily COVID-19 incidence, hospital census, deaths, proportion of cumulative incidence, and effective reproductive number, $R\textsubscript{eff}$. 
